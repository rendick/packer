...
while true; do
    echo "NvChad - theme for NeoVim"
    echo " "
    read -p "Do you wish to install NvChad?(yes or no) " yn
    case $yn in
        [Yy]* ) git clone https://github.com/NvChad/NvChad ~/.config/nvim --depth 1 ; nvim; break;; 
        [Nn]* ) exit;;
        * ) echo "Please answer yes or no.";;
    esac
done
...
echo -e "${ORANGE}Thank you $USER. You have already installed all packages for design! ${NC}"
...
echo -e "${ORANGE}Thank you $USER. You have already installed all packages for hacking! ${NC}"
